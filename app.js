...
function save(){
 try{localStorage.setItem(KEY,JSON.stringify(state))}catch(e){persistent=false}
...
function put(k,v){state.answers[k]=v;save()}
...
document.addEventListener("click",e=>{
 ...
 const action=actions[b.dataset.action];if(action)action(b);
});
const chapters=[
...
function render(){
 renderNav();
 $("main").innerHTML='<div class="eyebrow">Kapitel '+(state.chapter+1)+' · '+chapters[state.chapter][1]+' Minuten</div><div class="steps"><span>Erleben</span><span>Verstehen</span><span>Reflektieren</span><span>Einordnen</span><span>Gestalten</span></div>'+pages[state.chapter]()+
 ...
 save();if(state.chapter===1)updateFruit();if(state.chapter===2){updateRules();updateTokens()}
}
function go(i){if(i<0||i>7)return;state.chapter=i;save();render();$("main").focus();window.scrollTo({top:0,behavior:"auto"})}
...
function updateFruit(){
 ...
 if(w===null){$("fruitResult").textContent="Bitte 50 bis 400 g eingeben.";$("ruleResult").textContent="";$("outside").textContent="";return}
 const g=fruitGuess(fruit,w);
...
function resetFruit(){fruit=baseFruit.map(r=>({...r}));$("fruitWeight").value=160;$("threshold").value=140;$("fruitMsg").textContent="Ausgangsdaten wiederhergestellt.";updateFruit()}
...
const tokenOptions=[[["Bern",70],["Zürich",20],["Genf",10]],[["Kind",45],["Drache",35],["König",20]],[["Brot",45],["Müsli",35],["Suppe",20]],[[",",80],["und",12],["war",8]]];
const tokenFollow={
 Bern:[[".",85],[",",15]],Zürich:[[".",85],[",",15]],Genf:[[".",85],[",",15]],
 Kind:[[".",30],[",",70]],Drache:[[".",30],[",",70]],König:[[".",30],[",",70]],
 Brot:[[".",75],["und",25]],Müsli:[[".",75],["und",25]],Suppe:[[".",75],["und",25]],
 ",":[["sah",60],["hörte",40]],sah:[["ich",100]],hörte:[["ich",100]],
 ich:[["jemanden",55],["niemanden",45]],jemanden:[[".",100]],niemanden:[[".",100]],
 und:[["Obst",100]],Obst:[[".",100]],war:[[".",100]]
};
let tokenIndex=0,generated=[];
function optionsNow(){if(generated.length>=6||generated.at(-1)===".")return[];return generated.length?(tokenFollow[generated.at(-1)]||[[".",100]]):tokenOptions[tokenIndex]}
function tokenText(){return tokenStarts[tokenIndex]+generated.reduce((s,t)=>s+(/^[.,!?]$/.test(t)?"":" ")+t,"")}
function resetTokens(){tokenIndex=Number($("context").value);generated=[];updateTokens()}
...
function nextToken(){const opts=optionsNow();if(opts.length){generated.push(chooseToken(opts,$("sampling").value));updateTokens()}}
const prompts=[
...
async function copyPrompt(i){try{await navigator.clipboard.writeText(prompts[i][1]);$("copyStatus").textContent="Prompt kopiert."}catch(e){$("copyStatus").textContent="Automatisches Kopieren ist nicht verfügbar. Bitte markiere den Prompt und kopiere ihn manuell."}}
function promptCard(i){return'<h3>'+prompts[i][0]+'</h3><p>'+prompts[i][1]+'</p><button class="secondary" data-action="copy" data-index="'+i+'">Prompt kopieren</button>'}
const cards=[
...
function clearNotes(){
 ...
 state={answers:{},done:[],chapter:0};save();render();
}
...
function guideHTML(){return sheet("Moderationsleitfaden · 120 Minuten",'<p><strong>Leitfrage:</strong> Was kann die lernende Person danach selbst?</p>'+scheduleTable())+
 ...
 sheet("Quellen und Einordnung",sourcesHTML)}
...
const actions={
 ...
 addFruit,resetFruit,nextToken,resetTokens,copy:b=>copyPrompt(Number(b.dataset.index)),revealModel
};
