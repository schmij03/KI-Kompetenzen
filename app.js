...
const compass=[
 ...
 "Ich kann KI-Nutzungsregeln, Lösungen oder Lernsettings gemeinsam entwickeln, erproben und begründet verbessern."
];
...
// Opening exercise: the honest answer is often that the surface does not reveal the method.
const alltagOptions=[["regel","Feste Regeln"],["gelernt","Aus Daten gelernt"],["offen","Von aussen nicht entscheidbar"]];
const alltagLabels=Object.fromEntries(alltagOptions);
const alltagCases=[
 ["Ein Taschenrechner addiert zwei Zahlen.","regel","Die Rechenregeln stehen fest im Programm. Bei gleicher Eingabe kommt immer dasselbe heraus. Hier wird nichts aus Beispielen abgeleitet."],
 ["Ein Wecker klingelt um 6:30 Uhr.","regel","Eine Bedingung, eine Folge. Digital ist nicht dasselbe wie lernend."],
 ["Eine Foto-App findet alle Bilder, auf denen ein Hund zu sehen ist.","gelernt","Von Hand geschriebene Regeln für «Hund» funktionieren nicht zuverlässig. Solche Systeme werden mit sehr vielen Beispielbildern trainiert."],
 ["Ein Spamfilter sortiert eine E-Mail aus.","gelernt","Heutige Filter lernen aus grossen Mengen markierter Nachrichten und passen sich an. Frühe Filter arbeiteten mit festen Wortlisten. Dieselbe Aufgabe ist also mit beiden Ansätzen lösbar."],
 ["Ein Streamingdienst empfiehlt dir den nächsten Film.","offen","Empfehlungen können auf gelernten Mustern beruhen. Sie können aber auch aus einfachen Regeln entstehen, etwa «gleiches Genre» oder «am häufigsten gesehen». Von aussen ist das nicht zu unterscheiden."],
 ["Ein Chatfenster auf einer Website beantwortet deine Frage.","offen","Dahinter kann ein Entscheidungsbaum mit vorformulierten Antworten stehen oder ein Sprachmodell. Die Oberfläche sieht in beiden Fällen gleich aus."]
];
function alltagHTML(){
 const open=val("alltagOpen")===true;
 return '<div class="card"><h2>KI oder nicht KI?</h2><p>Entscheidet zuerst selbst, wie jedes System vermutlich arbeitet. Erst danach die Auflösung öffnen.</p><div class="grid">'+
 alltagCases.map(([text,answer,why],i)=>'<div class="card case"><p class="case-text">'+text+'</p>'+
  select("alltag"+i,"Wie arbeitet dieses System?",alltagOptions)+
  (open?'<p class="resolution"><strong>Unsere Einordnung: '+alltagLabels[answer]+'</strong><br>'+why+'</p>':"")+
  '</div>').join("")+'</div>'+
 (open?'<div class="note"><strong>Zwei der sechs Fälle lassen sich von aussen nicht entscheiden.</strong> «Daten → Verarbeitung → Output» beschreibt auch Systeme ohne KI. Entscheidend ist, wie die Verarbeitung zustande kommt, und genau das verrät eine Oberfläche meistens nicht.</div>':
 '<div class="actions"><button data-action="revealAlltag">Auflösung anzeigen</button></div>')+
 field("alltag","Bei welchem Fall wart ihr uneinig? Woran würdet ihr es festmachen?")+'</div>';
}
const cards=[
...
const domainTexts=[
 ...
 "KI-Systeme und ihre Nutzung aktiv mitgestalten: Regeln, Lösungen oder Nutzungsszenarien entwickeln, erproben und anhand von Rückmeldungen begründet verbessern."
];
function revealAlltag(){put("alltagOpen",true);render();$("main").scrollIntoView({block:"start"})}
...
// One source for the web chapter and the printed sheet.
const checkQuestions=[
 "Welche Aussage ist durch welche Quelle gestützt?",
 "Welche Perspektiven, Annahmen oder Gruppen fehlen?",
 "Welche Informationen habe ich eingegeben – und waren sie nötig?",
 "Welche Folgen könnte meine Verwendung des Outputs haben?",
 "Gibt es für diese Aufgabe eine ressourcenschonendere Alternative?"
];
...
const sourcesHTML='<p><a href="./unterrichtsideen.html#materialien">Ergänzende Unterrichtsmaterialien</a>: Code.org / CodeAI, Soekia, Quick, Draw!, AI Unplugged und IT2School. Die Unterrichtsideen verbinden externe Angebote mit eigenen Lernaufträgen und Lernbelegen.</p><p>Kompetenzmodell: Susanne Alles, Joscha Falck, Manuel Flick und Regina Schulz, «KI-Kompetenzen für Lehrende und Lernende», vorgestellt durch den DLH Zürich. Karten und Kurzbeschreibungen sind eigene didaktische Bearbeitungen, keine vollständige Wiedergabe der Originalmatrix.</p><p><a href="https://dlh.zh.ch/home/wb-kompass/kompetenzmodelle/ki-kompetenzen-fuer-lehrende-und-lernende">Modell und Originalgrafiken beim DLH Zürich</a></p><p><a href="https://kompassdigitalerwandel.ch/dk04/">Kompass digitaler Wandel: Informatik</a> – Grundlage für den Einstieg über Informationsverarbeitung.</p><p>OECD / Europäische Union (2026): <em>Lernende für das KI-Zeitalter befähigen – Ein KI-Kompetenzrahmen für die Primar- und Sekundarstufe</em>. Mit der bereitgestellten deutschen Original-PDF abgeglichen: S. 8–9 (Bereiche und Kompetenzverständnis), S. 19–24 (Wissen, Fähigkeiten/Fertigkeiten und Haltungen), S. 25 (Progression), S. 40–44 (aktive Mitgestaltung). <a href="https://doi.org/10.1787/65cd27d4-en">Originalpublikation (englisch)</a>. Eigene didaktische Zusammenfassung; keine Gleichsetzung der beiden Modelle.</p><p class="small">AILit-Grundlage: © OECD / Europäische Union, 2026, CC BY 4.0. Dies ist eine Bearbeitung einer Originalfassung der OECD und der Europäischen Union. Die in dieser Fassung geäußerten Meinungen und Argumente sollten nicht als offizielle Ansichten der OECD, ihrer Mitgliedsländer oder der Europäischen Union dargestellt werden.</p><p><a href="https://schmij03.github.io/ki-explained/">KI Explained</a> – didaktische Inspiration nach der bereitgestellten Beschreibung. Die Seite konnte bei der inhaltlichen Ausarbeitung nicht direkt geprüft werden. Übungen, Oberfläche und Texte wurden eigenständig entwickelt.</p>';
...
()=>'<h1>Was folgt aus den Experimenten?</h1><p class="lead">Formuliert eure eigenen Gedanken und vergleicht sie zu zweit. Ergänzt jeweils ein konkretes Beispiel.</p><div class="card">'+field("kann","KI kann gut …")+field("nicht","KI kann nicht zuverlässig …")+field("mensch","Deshalb muss der Mensch …")+'</div><div class="card"><h2>Fünf Fragen vor dem Übernehmen</h2><ol>'+checkQuestions.map(q=>'<li>'+q+'</li>').join("")+'</ol><p class="small">Die fünfte Frage stammt aus dem KI-Kompetenzrahmen von OECD und Europäischer Union. Betrieb und Entwicklung von KI-Systemen benötigen Energie, Rohstoffe und Wasser. Ob eine Nutzung angemessen ist, hängt auch davon ab, ob ein einfacheres Werkzeug denselben Zweck erfüllt.</p>'+field("pruefentscheidung","Eine Aussage oder Empfehlung, die ich nicht ungeprüft übernehmen würde – weil …")+'<details><summary>Mögliche Erkenntnissicherung</summary><p>KI kann Muster nutzen, Texte entwerfen und Varianten erzeugen. Sie liefert keine automatische Garantie für Wahrheit, Fairness oder Angemessenheit. Menschen müssen Ziele und Kriterien setzen, Ergebnisse prüfen und über ihre Verwendung entscheiden.</p><p>Zur Verantwortung gehört auch die Frage nach dem Aufwand: Nicht jede Aufgabe braucht ein grosses Modell.</p></details></div>',
...
const journalNames={
 ...
 partnercheck:"Partnerprüfung",fach:"Fach / Thema / Klasse im Zyklus 3",niveau:"Niveau",niveaubezug:"Bezug zur Originalmatrix",
...
alltagCases.forEach(([text],i)=>journalNames["alltag"+i]="KI oder nicht KI: "+text);
...
function canvasPaper(filled=false){
 ...
 lines("Fach / Thema / Klasse im Zyklus 3",v("fach"))+
...
function materialsHTML(){
 ...
 out+=sheet("KI oder nicht KI",'<p>Entscheidet für jedes System, wie es vermutlich arbeitet. Kreuzt an und notiert eure Begründung. Die Auflösung steht in der Lernumgebung.</p><table><thead><tr><th>System</th><th>'+alltagOptions.map(([,l])=>l).join('</th><th>')+'</th><th>Begründung</th></tr></thead><tbody>'+alltagCases.map(([text])=>'<tr style="height:18mm"><td>'+text+'</td>'+alltagOptions.map(()=>'<td>☐</td>').join("")+'<td></td></tr>').join("")+'</tbody></table>'+lines("Bei welchem Fall wart ihr uneinig? Woran würdet ihr es festmachen?"));
 ...
 out+=sheet("Prüfen und erklären",'<table><thead><tr><th>Aussage</th><th>Urteil</th><th>Quelle / Fundstelle / Begründung</th></tr></thead><tbody>'+[1,2,3].map(()=>'<tr style="height:30mm"><td></td><td></td><td></td></tr>').join("")+'</tbody></table><h2>Fünf Fragen vor dem Übernehmen</h2><ol>'+checkQuestions.map(q=>'<li>'+q+'</li>').join("")+'</ol>'+lines("KI kann gut …")+lines("KI kann nicht zuverlässig …")+lines("Deshalb muss der Mensch …"));
...
const actions={
 ...
 addFruit,resetFruit,undoFruit,nextToken,resetTokens,copy:b=>copyPrompt(Number(b.dataset.index)),revealModel,revealAlltag,chooseTask:b=>chooseTask(Number(b.dataset.index)),quiz:b=>checkQuiz(Number(b.dataset.index))
};
...
// The axis follows the data so that added examples stay distinguishable instead of
// collapsing into a fixed 50–400 g range.
function fruitDomain(values){
 const lo=Math.min(...values),hi=Math.max(...values),pad=Math.max((hi-lo)*.12,8);
 return{lo:lo-pad,hi:hi+pad};
}
function fruitTicks(lo,hi){
 const step=[1,2,5,10,20,25,50,100,200,500].find(s=>(hi-lo)/s<=6)||1000,out=[];
 for(let t=Math.ceil(lo/step)*step;t<=hi;t+=step)out.push(Math.round(t*100)/100);
 return out;
}
function drawFruit(w,g){
 // A phone renders the wide layout too small to read, so narrow screens get a taller box.
 const narrow=($("fruitChart").clientWidth||600)<430;
 const box=narrow?{w:360,h:250,x0:52,x1:344,apple:60,pear:128,axis:176,font:16}
 :{w:550,h:190,x0:60,x1:490,apple:47,pear:102,axis:135,font:14};
 const{lo,hi}=fruitDomain([...fruit.map(r=>r.w),w]);
 const x=n=>box.x0+(n-lo)/(hi-lo)*(box.x1-box.x0);
 $("fruitChart").innerHTML='<svg viewBox="0 0 '+box.w+' '+box.h+'" font-size="'+box.font+'" role="img" aria-labelledby="fruitChartTitle"><title id="fruitChartTitle">Gewichte der Trainingsbeispiele und Testfall bei '+w+' Gramm. Nächste Entfernung: '+g.distance+' Gramm.</title>'+
 '<text x="4" y="'+(box.apple+5)+'">Apfel</text><text x="4" y="'+(box.pear+5)+'">Birne</text>'+
 '<line x1="'+box.x0+'" y1="'+box.axis+'" x2="'+box.x1+'" y2="'+box.axis+'" stroke="#718392"/>'+
 fruitTicks(lo,hi).map(t=>'<line x1="'+x(t)+'" y1="'+box.axis+'" x2="'+x(t)+'" y2="'+(box.axis+6)+'" stroke="#718392"/><text x="'+x(t)+'" y="'+(box.axis+box.font+10)+'" text-anchor="middle">'+t+'</text>').join("")+
 '<text x="'+(box.x0+box.x1)/2+'" y="'+(box.h-6)+'" text-anchor="middle">Gewicht in Gramm</text>'+
 '<line x1="'+x(w)+'" y1="16" x2="'+x(w)+'" y2="'+(box.axis-5)+'" stroke="#172d41" stroke-width="2" stroke-dasharray="4 4"/>'+
 fruit.map(r=>{const px=x(r.w),py=r.t==="Apfel"?box.apple:box.pear;const common=' fill="'+(r.t==="Apfel"?"#197448":"#075f9c")+'" stroke="'+(g.near.includes(r)?"#111":"white")+'" stroke-width="'+(g.near.includes(r)?3:1)+'"';
...
window.addEventListener("resize",()=>{
 ...
 clearTimeout(chartResize);chartResize=setTimeout(updateFruit,150);
});
function undoFruit(){
...
Object.entries(state.answers).forEach(([key,value])=>{
 let valid=true;
 if(/^(domain[0-3]|modelOpen|alltagOpen|quizChecked[0-2])$/.test(key))valid=typeof value==="boolean";
 else if(/^alltag[0-5]$/.test(key))valid=value===""||alltagOptions.some(([v])=>v===value);
